...
func expandDynamoDbTableItemAttributes(input string) (map[string]*dynamodb.AttributeValue, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Error deserializing DynamoDB item: %s", err)
	}
...
func flattenDynamoDbTableItemAttributes(attrs map[string]*dynamodb.AttributeValue) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Error encoding DynamoDB item: %s", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("Error decoding DynamoDB item: %s", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("Error re-encoding DynamoDB item: %s", err)
	}
